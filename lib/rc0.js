...
// write counter, not a fixed marker. The factory ships the pair at "8"/"9";
// a save on the pedal stamps the freshly written bank one generation past
// the highest on the card, bumping the raw byte with no decimal carry
// ("8", "9", ":", ";", … — observed on hardware: MEMORY1=0x3a next to
// MEMORY2=0x39). A structurally broken trailer triggers "LOOPER DATA READ
// ERR" on boot; any counter value is legal.
...
// trailer byte is really a write-generation counter — see setTailGeneration.
...
// silently rewritten.
export function setTailGeneration(text, generation) {
  if (!Number.isInteger(generation) || generation < 0 || generation > 0xff)
    throw new Error(`generation must be a byte 0..255, got ${generation}`);
  if (tailMarker(text) === null)
    throw new Error('unrecognized trailer after </database>; refusing to rewrite it');
  return text.slice(0, -4) + String.fromCharCode(generation) + '\0\0\0';
}
...
export function setTailMarker(text, fileNo) {
  ...
  return setTailGeneration(text, marker);
}
