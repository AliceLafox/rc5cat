...
// back up, edit MEMORY1's content, write it to BOTH memory files with
// advancing write-generation trailers, verify by re-reading, sweep
// AppleDouble junk.
...
export function writeMemoryPair(volume, text, { skipBackup = false, backupDir } = {}) {
  ...
  let base = 0x37; // one below '8': a fresh volume lands on the factory pair
  for (const fileNo of [1, 2]) {
    try {
      const marker = rc0.tailMarker(readLatin1(vol.memoryPath(volume, fileNo)));
      if (marker !== null) base = Math.max(base, marker);
    } catch {
      // unreadable bank: no generation to continue from
    }
  }
  for (const fileNo of [1, 2]) {
    const withTail = rc0.setTailGeneration(text, (base + fileNo) & 0xff);
...
export function clear(volume, slots, { keepName = false, trash = true, trashDir, ...opts } = {}) {
  let text = readMemory(volume);
  ...
  return { trashed, deleted, ...written };
}
...
export function doctor(volume) {
  ...
  const hexByte = (b) => `0x${b.toString(16).padStart(2, '0')}`;
  ...
  for (const fileNo of [1, 2]) {
    try {
      ...
      if (marker === null)
        findings.push({
          level: 'error',
          message: `MEMORY${fileNo}.RC0 trailer is malformed — causes LOOPER DATA READ ERR`,
        });
      else generations[fileNo] = marker;
    } catch (e) {
      findings.push({ level: 'error', message: `MEMORY${fileNo}.RC0: ${e.message}` });
    }
  }

  if (1 in generations && 2 in generations) {
    ...
    const delta = (generations[1] - generations[2]) & 0xff;
    if (delta !== 0 && delta !== 1 && delta !== 0xff)
      findings.push({
        level: 'warn',
        message: `MEMORY write generations ${hexByte(generations[1])} / ${hexByte(generations[2])} ` +
                 'are more than one step apart — unexpected state, consider a backup before writing',
      });
  }

  if (texts[1] && texts[2] && rc0.splitFile(texts[1]).document !== rc0.splitFile(texts[2]).document) {
    const note = 1 in generations && 2 in generations
      ? ` (write generations ${hexByte(generations[1])} / ${hexByte(generations[2])})`
      : '';
    findings.push({
      level: 'info',
      message: `MEMORY1 and MEMORY2 differ${note} — normal right after a save on the pedal; a reboot reconciles the pair`,
    });
  }
