...
import { sweepJunk, listSlotWavs } from './volume.js';
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/api/state')
        return sendJson(200, state(volume));
      ...
      if (req.method === 'POST') {
        ...
        if (url.pathname === '/api/clear') {
          const { slot, keepName } = JSON.parse((await readBody(req, 4096)).toString('utf8'));
          const { trashed } = commands.clear(volume, [slot], { keepName: Boolean(keepName), trashDir, ...opts });
          return sendJson(200, { trashed, ...state(volume) });
        }
        if (url.pathname === '/api/clean') {
          const swept = sweepJunk(volume);
          return sendJson(200, { swept, ...state(volume) });
        }
